...
public class winlvl1 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (File.Exists(Application.persistentDataPath + "/player.Playscore1"))
        {
            lvl1hillData data = Savelvl1.LoadData();
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.tag == "Player")
        {
            ...
            if (Mathf.Round(Highscore) > Mathf.Round(mayHighscore))
            {
                float Highscorenow;
            ...
            else if (Highscore < mayHighscore && setdata)
            {
                float minutsnow;
                float secondsnow;
